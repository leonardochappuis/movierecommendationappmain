...
// Define a type for watchlist items
...
export default function MovieRecommendationCard() {
  ...
  // Helper to check if a movie is in watchlist
  const isInWatchlist = (movieId: number) => {
    return watchlist.some((item) => item.movie.id === movieId)
  }

  // Get watchlist movies in order (for display)
  const getOrderedWatchlistMovies = () => {
    return [...watchlist].sort((a, b) => a.addedAt - b.addedAt).map((item) => item.movie)
  }

  // Add to Watchlist
  const handleToggleWatchlist = (movie: any) => {
    let undo = false
    const movieInWatchlist = watchlist.find((item) => item.movie.id === movie.id)

    if (movieInWatchlist) {
      // Store the timestamp before removing
      const timestamp = movieInWatchlist.addedAt
      ...
      toast("Removed from watchlist", {
        ...
        action: {
          ...
          onClick: () => {
            undo = true
            // Restore the movie with its original timestamp
            setWatchlist((prev) => [...prev, { movie, addedAt: timestamp }])
          },
        },
      })
      ...
      // Add to watchlist with current timestamp
      setWatchlist((prev) => [...prev, { movie, addedAt: Date.now() }])
  ...
  // Get ordered watchlist movies
  const watchlistMovies = getOrderedWatchlistMovies()
